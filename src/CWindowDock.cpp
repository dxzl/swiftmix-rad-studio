// http://freesourcecode.net/cprojects/91587/sourcecode/CWindowDock.cpp#.WU0tNekpC70
//
...
void CWindowDock::WindowMoved(HWND hwnd)
{
    ...
    if(hwnd != m_Parent)
    {
        ...
        {
//            MoveWindow(hwnd, rcNew.left, rcNew.top, -1, -1, true);
            SetWindowPos(hwnd, 0, rcNew.left, rcNew.top, 0, 0, SWP_NOZORDER | SWP_NOSIZE | SWP_DEFERERASE);
        }
        else
        {
    ...
    else
    {
        ...
        for(i = m_ChildList.begin(); i != m_ChildList.end(); i++)
        {
            ...
            //Move the window
            SetWindowPos(child->GetHandle(), 0, rcNew.left, rcNew.top, 0, 0, SWP_NOZORDER | SWP_NOSIZE | SWP_DEFERERASE);

            //MoveWindow(child->GetHandle(), rcNew.left, rcNew.top, -1, -1, true);
        }
    }
